...
at https://github.com/PonyGE/PonyGE2/issues/130."""
    ...
    def get_first_categorical_feature(self, max_different_values = 10):
        ...
        if type(self.training_in) == np.ndarray:
            if not issubclass(self.training_in.dtype.type, np.number):
                return 0

        elif type(self.training_in) == pd.DataFrame:
            for i in range(self.training_in.shape[1]):
                if self.is_ithfeature_categorical(i, max_different_values):
                    return i

        else:
            warnings.warn('Unrecognized dataset. Not numpy.ndarray, nor pandas.DataFrame')
    ...
    def get_first_numerical_feature(self, min_different_values = 11):
        ...
        if type(self.training_in) == np.ndarray:
            if issubclass(self.training_in.dtype.type, np.number):
                return 0
